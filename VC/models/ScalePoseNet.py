...
class FeatureBlock(nn.Module):
    def __init__(self, in_channels, out_channels, **kargs):
        ...
        self.bottle = nn.Conv2d(in_channels * 7, out_channels, kernel_size=1, bias=False)
        self.relu = nn.ReLU(True)

        self.down_1 = self._downsampling(BasicBlock, in_channels, in_channels * 2)
        self.down_2 = self._downsampling(BasicBlock, in_channels * 2, in_channels * 4)
        ...
        self.up_1 = self._upsampling(UpsampleBlock, in_channels=in_channels * 2, out_channels=in_channels * 2,
                                     scale_factor=2)
        self.up_2 = self._upsampling(UpsampleBlock, in_channels=in_channels * 4, out_channels=in_channels * 4,
                                     scale_factor=4)
...
class ScalePoseNet(nn.Module):
    def __init__(self, opt, criterion=JointsMSELoss(use_target_weight=False)):
        ...
        self.conv = nn.Conv2d(3, self.num_channels, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        for i in range(self.num_blocks):
            self.add_module('fb_{}'.format(i), FeatureBlock(self.num_channels, self.num_channels))
    ...
    def get_loss(self, target_heatmaps):
        ...
        for i, heatmaps in enumerate(self.heatmaps_list):
            loss += i * self.criterion(heatmaps, target_heatmaps) / len(self.heatmaps_list)
