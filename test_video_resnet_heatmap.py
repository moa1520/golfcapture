...
from dataloader import Normalize, Normalize_pose, SampleVideo, SampleVideo_repeat, ToTensor, ToTensor_pose
...
if __name__ == '__main__':
    path = 'data/total_videos/bad_side_swing0253.mp4'
    save_dict_path = 'checkpoints/plan1_repeat/swingnet_4500(best).pth.tar'
    save_dict_pose_path = '/home/tk/Desktop/ktk/VC_revised/checkpoints/ScalePoseNet_latest'
    seq_length = 64
    heatmap_size = 56
    input_size = 224
    ...
    ds = SampleVideo_repeat(path, transform=transforms.Compose([ToTensor_pose(),
                                                                Normalize_pose([0.485, 0.456, 0.406],
                                                                               [0.229, 0.224, 0.225])]), input_size=input_size, posenet_input_size=512)

    dl = DataLoader(ds, batch_size=1, shuffle=False, drop_last=False)

    model_detection = Plan1_repeat(pretrain=True, width_mult=1, lstm_layers=1, lstm_hidden=256, bidirectional=True,
                                   dropout=False, heatmap_size=heatmap_size).to(device=device)
    model_posenet = ScalePoseNet(opt).to(device=device).to(device=device)
    ...
    for sample in dl:
        ...
        for i in range(images.shape[1]):
            ...
            ).numpy() * heatmap_size, heatmap_size, sigma=3))
        ...
        while batch * seq_length < images.shape[1]:
            ...
            logits = model_detection(image_batch.cuda(), heatmap_batch.cuda())
